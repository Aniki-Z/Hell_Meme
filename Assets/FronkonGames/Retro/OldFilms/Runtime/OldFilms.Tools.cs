...
namespace FronkonGames.Retro.OldFilms
{
  ...
  public sealed partial class OldFilms
  {
    ...
    private static readonly OldFilms[] NoEffects = new OldFilms[0];
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    public static OldFilms Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          for (int i = 0; i < scriptableRendererData.rendererFeatures.Count; ++i)
          {
            if (scriptableRendererData.rendererFeatures[i] is OldFilms)
              return scriptableRendererData.rendererFeatures[i] as OldFilms;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static OldFilms[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is OldFilms)
                  effects.Add(feature as OldFilms);
          }
